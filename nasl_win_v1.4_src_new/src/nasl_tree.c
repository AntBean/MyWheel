...
{
  tree_cell	*p = emalloc(sizeof(tree_cell));
  int		i;
...
dump_cell_val(const tree_cell* c)
{
  ...
  if (c == NULL)
    ...
    switch(c->type)
      ...
      case CONST_INT:
    if (!c->x.i_val) break;
	  snprintf(txt, sizeof(txt)-1, "%d", c->x.i_val);
	break;
      case CONST_STR:
      case CONST_DATA:		/* Beurk */
    if (!c->x.str_val) break;
	if (c->size >= sizeof(txt) + 2)
	  {
	    snprintf(txt, sizeof(txt)-1, "\"%s", c->x.str_val);
	...
	break;
      default:
	  snprintf(txt, sizeof(txt)-1, "???? (%s)", nasl_type_name(c->type));
	break;
      }
  return txt;
}
...
nasl_type_name(int t)
{
  ...
  if (t >= 0 || t < sizeof(node_names) / sizeof(node_names[0]))
    sprintf(txt, "%s (%d)", node_names[t], t);
  else
    sprintf(txt, "*UNKNOWN* (%d)", t);
  return txt;
}
...
get_line_nb(const tree_cell* c)
{
  ...
  if (c == NULL || c == FAKE_CELL || c->line_nb <= 0)
    return "";
  snprintf(txt, sizeof(txt)-1, " at or near line %d ", c->line_nb);
  return txt;
}
