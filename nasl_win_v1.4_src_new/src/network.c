      ...
      for (t = 0; total < max_len && (timeout <= 0 || t < timeout); )
	{
	  ...
	  if(select(realfd + 1, &fdr, NULL, NULL, timeout > 0 ? &tv : NULL) <= 0)
	  ...
	  else
	    {
		  ...
		  ret = recv(realfd, buf + total, max_len - total, 0);
 ...
 int length, i_opt;
{
   ...
   do
 {
       ...
       else if ( e < 0 && err == WSAEINTR ) continue;
       else break;
     }
   while (n <= 0 && err == WSAEINTR);
   if (n < 0)
 ...
 int timeout;
{
  ...
  ret = open_socket(&addr, port, type, protocol, timeout);
  add_socket(args, ret);
  return ret;
}
...
int os_send(int soc, void * buf, int len, int opt )
{
 ...
 for ( n = 0 ; n < len ; ) 
 {
  ...
  if ( e < 0 && WSAGetLastError() == WSAEINTR ) continue; 
