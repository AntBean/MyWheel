...
get_var_name(anon_nasl_var *v)
{
  ...
  if (v->av_name != NULL)
    return v->av_name;
#endif
  snprintf(str, sizeof(str)-1, "[%08x]", (int)v);
  return str;
}
  ...
  if (a->num_elt != NULL)
      ...
      if ((u = a->num_elt[i]) != NULL && u->var_type != VAR2_UNDEF)
	{
	  ...
	  switch (u->var_type)
	    {
	    case VAR2_INT:
	      snprintf(s+n, len - n - 1, "%d: %d", i, u->v.v_int);
	      n += strlen(s + n);
	      break;
	    case VAR2_STRING:
	    case VAR2_DATA:
		...
		{
		  snprintf(s+n, len - n - 1, "%d: '%s'", i, u->v.v_str.s_val);
		  n += strlen(s + n);
		}
	      else
		{
		  snprintf(s+n, 70 - 1, "%d: '%s", i, u->v.v_str.s_val);
		  n += strlen(s + n);
		  n += sprintf(s+n, "'...");
		}
	      break;
	    default:
	      snprintf(s+n, len-n-1, "%d: ????", i);
	      n += strlen(s + n);
	      break;
	    }
	}
	...
	if (v->u.var_type != VAR2_UNDEF)
	  {
	    ...
	    switch (u->var_type)
	      {
	      case VAR2_INT:
		n += snprintf(s+n, len - n - 1, "%s: %d", v->var_name, u->v.v_int);
		break;
	      case VAR2_STRING:
	      case VAR2_DATA:
		if (u->v.v_str.s_siz < 64)
		  {
		    snprintf(s+n, len - n - 1, "%s: '%s'", v->var_name, u->v.v_str.s_val);
		    n += strlen(s + n);
		  }
		else
		  {
		    snprintf(s+n, 70+l-1, "%s: '%s", v->var_name, u->v.v_str.s_val);
		    n += strlen(s + n);
		    n += sprintf(s+n, "'...");
		  }
		break;
	      default:
		snprintf(s+n, len-n-1, "%s: ????", v->var_name);
		n += strlen(s + n);
		break;
	      }
	  }
...
const char*	var2str(const anon_nasl_var* v)
{
  ...
  switch (v->var_type)
    {
    case VAR2_INT:
      snprintf(s1, sizeof(s1)-1, "%d", v->v.v_int);
