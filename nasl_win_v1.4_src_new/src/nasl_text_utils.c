...
tree_cell* nasl_hex(lex_ctxt * lexic)
{
 ...
 if(v == -1)
  ...
  snprintf(ret, sizeof(ret)-1, "0x%02x", (unsigned char)v);
  retc = alloc_tree_cell(0, NULL);
  retc->type = CONST_STR;
...
tree_cell* nasl_hexstr(lex_ctxt * lexic)
{
 ...
 if(s == NULL)
  ...
  {
   char tmp[3] = {0};
   snprintf(tmp, sizeof(tmp)-1, "%02x", (unsigned char)s[i]);
   strcat(ret, tmp);
  }
...
tree_cell* nasl_crap(lex_ctxt* lexic)
{
    ...
    else
    {
        ...
        if(data_len != 1)
        {
            ...
            else if(len - data_len >= 0)
                memcpy(retc->x.str_val + (len - data_len), data, data_len);
        }
        else if(len > 0) retc->x.str_val[ len - 1 ] = data[0];
    }
    retc->x.str_val[len] = '\0';
    return retc;
}
