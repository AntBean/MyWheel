...
static int safe_copy(char * str, char * dst, int sz, char * path, char * item)
{
 ...
 if(strlen(str) >= sz)
 {
  fprintf(stderr, "safe_copy error: %s (%ld) is too long - %s\n", item, strlen(str), str);
  return -1;
 }
 strcpy(dst, str);
 return 0;
...
static char sys_store_dir[MAX_PATH+1];
static char usr_store_dir[MAX_PATH+1];
...
int store_init_sys(char * dir)
{
 ...
 snprintf(sys_store_dir, sizeof(sys_store_dir), "%s\\cache", dir);
 if((mkdir(sys_store_dir) < 0) && (errno != EEXIST))
 {
...
int store_init_user(char * dir)
{
 ...
 snprintf(usr_store_dir, sizeof(usr_store_dir), "%s\\cache", dir);
 if((mkdir(usr_store_dir) < 0) && (errno != EEXIST))
 {
...
{
 char desc_file[MAX_PATH+1];
 char plug_file[MAX_PATH+1];
 char * str;
 char store_dir[MAX_PATH+1];
 struct plugin p;
 ...
 snprintf(desc_file, sizeof(desc_file), "%s\\cache\\%s", dir, file);
 ...
 snprintf(store_dir, sizeof(store_dir), "%s\\cache", dir);
 ...
 arg_add_value(ret, "preferences", ARG_PTR, -1, prefs);
...
struct arglist * store_plugin(struct arglist * plugin, char * file)
{
 ...
 if (!strlen(dir)) {
    get_work_dir(path, NULL);
    if (strlen(path) && path[strlen(path)-1]=='\\')
        path[strlen(path)-1] = '\0';
    store_init_sys(path);
    store_init_user(path);
 }
 ...
 fd = open(desc_file, O_RDWR|O_CREAT|O_TRUNC|O_BINARY, 0644);
 if(fd < 0)
 { 
  return NULL;
 }
 ...
 arg_add_value(ret, "preferences", ARG_PTR, -1, arg_get_value(plugin, "preferences"));
