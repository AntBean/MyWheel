...
again: 
 ...
 if(e <= 0) {
    ...
    if(e < 0 && WSAGetLastError() == WSAEINTR)goto again;
    return e;
    }
 r ++;
 if(buf[r-1] == '\n'){
...
again:       
    ...
    if(e <= 0)
    {
     ...
     if(e < 0 && err == WSAEINTR)goto again;
...
again2:    
    ...
    {
     DWORD err = WSAGetLastError();
     if(e < 0 && err == WSAEINTR)goto again2;
     else break;
    }
...
again:
     ...
     /*  pcap_restart(NULL);    */    
       if ( pcap_compile(pcap, &clients[i].bpf_filter, buf, 0, netmask) < 0 )
	 {
...
recv_again: 
 ...
 {
  DWORD err = WSAGetLastError();
  if(e < 0 && err == WSAEINTR)goto recv_again;
  else return -1;
 }
...
recv_again2:  
  ...
  err = WSAGetLastError();
  if(e < 0 && err == WSAEINTR)goto recv_again2;
