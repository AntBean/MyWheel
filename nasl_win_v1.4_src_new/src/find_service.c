...
int find_service_plugin_init(desc)
 struct arglist * desc;
{ 
 ...
 plug_set_name(desc, EN_NAME, "english");
     ...
     const char		*proto;
{
...
#endif
  /* Old "magical" key set */
  snprintf(k, sizeof(k)-1, "Services/%s", proto);
   ...
   */
  snprintf(k, sizeof(k)-1, "Known/tcp/%d", port);
 ...
 char * buffer;
{
 ...
 if(port==593)
 {
  ...
  snprintf(ban, sizeof(ban)-1, "http-rpc-epmap/banner/%d", port);
  plug_set_key(desc, ban, ARG_STRING, buffer);
 }
 else
 {
  register_service(desc, port, "ncacn_http");
  snprintf(ban, sizeof(ban)-1, "ncacn_http/banner/%d", port);
  plug_set_key(desc, ban, ARG_STRING, buffer);
 }
}
 ...
 char * buffer;
{
 ...
 snprintf(ban, sizeof(ban)-1, "vnc/banner/%d", port);
 plug_set_key(desc, ban, ARG_STRING, buffer);
}
 ...
 char * buffer;
{
 ...
 snprintf(ban, sizeof(ban)-1, "nntp/banner/%d", port);
 plug_set_key(desc, ban, ARG_STRING, buffer);
 snprintf(ban, sizeof(ban)-1, "An NNTP server is running on this port%s",
 ...
 char * buffer;
{
   ...
   snprintf(ban, sizeof(ban)-1, "www/banner/%d", port);
   plug_set_key(desc, ban, ARG_STRING, buffer);
   snprintf(ban, sizeof(ban)-1, "A web server is running on this port%s\n\
Here is its banner : \n%s",
     ...
     char		*buffer;
{
  ...
  snprintf(ban, sizeof(ban)-1, "www/banner/%d", port);
  plug_set_key(desc, ban, ARG_STRING, buffer);
  snprintf(ban, sizeof(ban)-1, "A Gnutella servent is running on this port%s",
	  get_encaps_through(trp));
  post_note(desc, port, ban);
}
#endif
 ...
 char * buffer;
{
   ...
   snprintf(ban, sizeof(ban)-1, "realserver/banner/%d", port);
   plug_set_key(desc, ban, ARG_STRING, buffer);
    
   snprintf(ban, sizeof(ban)-1, "A RealMedia server is running on this port%s",
	     get_encaps_through(trp));
   post_note(desc, port, ban);
}
 ...
 char * buffer;
{
 ...
 snprintf(ban, sizeof(ban)-1, "smtp/banner/%d", port);
 plug_set_key(desc, ban, ARG_STRING, buffer);
 ...
 
 {
  ...
  if(t)t[0]=0;
  snprintf(report, 255 + strlen(buffer) - 1, "An SMTP server is running on this port%s\n\
Here is its banner : \n%s",
	  get_encaps_through(trp), buffer);
 ...
 char * buffer;
{
 ...
 if(buffer != NULL)
 {
  ...
  snprintf(ban, sizeof(ban)-1, "ftp/banner/%d", port);
  plug_set_key(desc, ban, ARG_STRING, buffer);
 }
 ...
 if(buffer != NULL)
 {
  ...
  if(t != NULL )
  	t[0]= '\0';
  snprintf(report, 255 + strlen(buffer) - 1, "An FTP server is running on this port%s.\n\
Here is its banner : \n%s",
	  get_encaps_through(trp), buffer);
   post_note(desc, port, report);
   efree(&report);
 ...
 {
  char report[255] = {0};
  snprintf(report, sizeof(report)-1, "An FTP server is running on this port%s.",
  	get_encaps_through(trp));
  post_note(desc, port, report);
 }
}
 ...
 char * buffer;
{
 ...
 snprintf(ban, sizeof(ban)-1, "A HTTP proxy is running on this port%s\n\
Here is its banner : \n%s",
	  get_encaps_through(trp), buffer);
 post_note(desc, port, ban);
}
 ...
 char * buffer;
{
 ...
 if(!strcmp(buffer2, "+ok"))
  ...
  snprintf(ban, sizeof(ban)-1, "pop1/banner/%d", port);
  plug_set_key(desc, ban, ARG_STRING, buffer);
  }
  else if(strstr(buffer2, "pop2"))
   {
   register_service(desc, port, "pop2");
   snprintf(ban, sizeof(ban)-1, "pop2/banner/%d", port);
   plug_set_key(desc, ban, ARG_STRING, buffer);
   post_note(desc, port, "a pop2 server is running on this port");
    ...
    snprintf(ban, sizeof(ban)-1, "pop3/banner/%d", port);
    plug_set_key(desc, ban, ARG_STRING, buffer);
 ...
 char * buffer;
{
 ...
 snprintf(ban, sizeof(ban)-1, "imap/banner/%d", port);
 plug_set_key(desc, ban, ARG_STRING, buffer);
   {
     snprintf(ban, sizeof(ban)-1, "An IMAP server is running on this port%s",
	     get_encaps_through(trp));
	     post_note(desc, port, ban);
   }
}
	...
	int trp;
{
 ...
 post_info(desc, port, "A rsync server is running on this port");
}
 ...
 char * buffer;
{
 ...
 register_service(desc, port, "telnet");
   {
     snprintf(ban, sizeof(ban)-1, "A telnet server seems to be running on this port%s\n",
	  get_encaps_through(trp));
     post_note(desc, port, ban);
   }
}
 ...
 char * buffer;
{
 ...
 snprintf(ban, sizeof(ban)-1, "linuxconf/banner/%d", port);
 plug_set_key(desc, ban, ARG_STRING, buffer);
     ...
     int		port, trp;
{
  ...
  snprintf(tmp, sizeof(tmp)-1, "A finger server seems to be running on this port%s\n\
Here is its banner : \n%s",
     ...
     int		port, trp;
{
  ...
  snprintf(tmp, sizeof(tmp)-1, "vtun/banner/%d", port);
  ...
  if(banner == NULL)
  {
  snprintf(tmp, sizeof(tmp)-1, "A VTUN server seems to be running on this port%s",
	  get_encaps_through(trp));
  }
  else  snprintf(tmp, sizeof(tmp)-1, "A VTUN server seems to be running on this port%s\nHere is its banner:\n%s\n",
	  get_encaps_through(trp), banner);
     ...
     int		port, trp;
{
  ...
  snprintf(tmp, sizeof(tmp)-1, "uucp/banner/%d", port);
  plug_set_key(desc, tmp, ARG_STRING, (char*)banner);
  
  register_service(desc, port, "uucp");
  
  snprintf(tmp, sizeof(tmp)-1, "An UUCP server seems to be running on this port%s",
	  get_encaps_through(trp));
  post_note(desc, port, tmp);
}
     ...
     int		port, trp;
{
  ...
  snprintf(tmp, sizeof(tmp)-1, "A LPD server seems to be running on this port%s",
	  get_encaps_through(trp));
  post_note(desc, port, tmp);
}
     ...
     int		port, trp;
{
  ...
  snprintf(tmp, sizeof(tmp)-1, "A LysKOM server seems to be running on this port%s",
	  get_encaps_through(trp));
  post_note(desc, port, tmp);
}
     ...
     int		port, trp;
{
  ...
  snprintf(tmp, sizeof(tmp)-1, "A PH server seems to be running on this port%s",
	  get_encaps_through(trp));
  post_note(desc, port, tmp);
}
     ...
     int		port, trp;
{
  ...
  snprintf(tmp, sizeof(tmp)-1, "A time server seems to be running on this port%s",
	  get_encaps_through(trp));
  post_note(desc, port, tmp);
}
     ...
     int		port, trp;
{
  ...
  snprintf(tmp, sizeof(tmp)-1, "An iPlanet ENS (Event Notification Server) seems to be running on this port%s",
	  get_encaps_through(trp));
  post_note(desc, port, tmp);
}
     ...
     int		port, trp;
{
 ...
 snprintf(tmp, sizeof(tmp)-1, "a Citrix server seems to be running on this port%s", 
   get_encaps_through(trp));
 post_note(desc, port, tmp);
}
	...
	int port, trp;
{
 ...
 snprintf(tmp, sizeof(tmp)-1,"A GIOP-enabled service is running on this port%s", 
 	get_encaps_through(trp));
 ...
 char * buffer;
{
 ...
 snprintf(ban, sizeof(ban)-1, "exchg-routing/banner/%d", port);
 plug_set_key(desc, ban, ARG_STRING, buffer);
   {
     snprintf(ban, sizeof(ban)-1, "A Microsoft Exchange routing server is running on this port%s",
	     get_encaps_through(trp));
	     post_note(desc, port, ban);
   }
}
 ...
 char * buffer;
{
 ...
 snprintf(msg, sizeof(msg)-1, "A tcpmux server seems to be running on this port%s",
	     get_encaps_through(trp));
 ...
 char * buffer;
{
 ...
 snprintf(msg, sizeof(msg)-1, "A SNMP Multiplexer (smux) seems to be running on this port%s",
	     get_encaps_through(trp));
  post_note(desc, port, msg);
}
	...
	int port, trp;
{
 ...
 snprintf(tmp, sizeof(tmp)-1, "A LISa daemon is running on this port%s", 
 	get_encaps_through(trp));
 ...
 char * buffer;
{
 ...
 post_note(desc, port, "An MSDTC server is running on this port");
}
 ...
 char * buffer;
{
 ...
 snprintf(ban, sizeof(ban)-1, "pop3pw/banner/%d", port);
 plug_set_key(desc, ban, ARG_STRING, buffer);
 snprintf(ban, sizeof(ban)-1, "A pop3pw server is running on this port%s", get_encaps_through(trp));
 post_note(desc, port, ban);
}
 ...
 char * buffer;
{
 ...
 snprintf(ban, sizeof(ban)-1, "whois++/banner/%d", port);
 plug_set_key(desc, ban, ARG_STRING, buffer);
 snprintf(ban, sizeof(ban)-1, "A whois++ server is running on this port%s", get_encaps_through(trp));
 post_note(desc, port, ban);
}
 ...
 char * buffer;
{
 ...
 snprintf(ban, sizeof(ban)-1, "mon/banner/%d", port);
 plug_set_key(desc, ban, ARG_STRING, buffer);
 snprintf(ban, sizeof(ban)-1, "A mon server is running on this port%s", get_encaps_through(trp));
 post_note(desc, port, ban);
}
 ...
 char * buffer;
{
 ...
 plug_set_key(desc, ban, ARG_STRING, buffer);
 snprintf(ban, sizeof(ban)-1, "A CheckPoint FW1 SecureRemote or FW1 FWModule server is running on this port%s", get_encaps_through(trp));
 post_note(desc, port, ban);
}
 ...
 char * buffer;
{
 ...
 plug_set_key(desc, ban, ARG_STRING, buffer);
 snprintf(ban, sizeof(ban)-1, "A PsyBNC IRC proxy is running on this port%s", get_encaps_through(trp));
 post_note(desc, port, ban);
}
 ...
 char * buffer;
{
 ...
 plug_set_key(desc, ban, ARG_STRING, buffer);
 snprintf(ban, sizeof(ban)-1, "A shoutcast server is running on this port%s", get_encaps_through(trp));
 post_note(desc, port, ban);
}
 ...
 char * buffer;
{
 ...
 plug_set_key(desc, ban, ARG_STRING, buffer);
 snprintf(ban, sizeof(ban)-1,"An AdsGone (a popup banner blocking server) is running on this port%s", get_encaps_through(trp));
 post_note(desc, port, ban);
}
 ...
 char * buffer;
{
 ...
 snprintf(ban, sizeof(ban)-1, "acap/banner/%d", port);
 plug_set_key(desc, ban, ARG_STRING, buffer);
   {
     snprintf(ban, sizeof(ban)-1, "An ACAP server is running on this port%s",
	     get_encaps_through(trp));
	     post_note(desc, port, ban);
   }
}
 ...
 char * buffer;
{
 ...
 snprintf(ban, sizeof(ban)-1, "A nagiosd server is running on this port%s",
	     get_encaps_through(trp));
	     post_note(desc, port, ban);
 ...
 char * buffer;
{
 ...
 snprintf(ban, sizeof(ban)-1, "A teamspeak2 server is running on this port%s",
	     get_encaps_through(trp));
	     post_note(desc, port, ban);
 ...
 char * buffer;
{
 ...
 snprintf(ban, sizeof(ban)-1, "A WEBSM server is running on this port%s",
	     get_encaps_through(trp));
	     post_note(desc, port, ban);
 ...
 char * buffer;
{
 ...
 snprintf(ban, sizeof(ban)-1, "An OFA/Express server is running on this port%s",
	     get_encaps_through(trp));
	     post_note(desc, port, ban);
     ...
     int		port, delta;
{
  ...
  snprintf(msg, sizeof(msg)-1, "The service closed the connection after %d seconds without sending any data\n\
...
{
    char tmp[1600] = {0}, *norm = NULL;
    char svcname[MAX_PATH] = {0};

  plug_set_key(desc, "Services/unknown", ARG_INT,(void*) port);
  snprintf(tmp, sizeof(tmp)-1, "unknown/banner/%d", port);
  plug_set_key(desc, tmp, ARG_STRING, (char*)banner);

  nessus_get_svc_name(desc, port, "tcp", svcname);
  if (strlen(svcname) && stricmp(svcname, "unknown")!=0) {
      norm = svcname;
  }
  ...
  /*switch (port)
    ...
    case 139:   norm = "NETBIOS"; break;
    case 220:	norm = "IMAP3"; break;
    case 443:	norm = "HTTPS"; break;
    case 445:   norm = "Microsoft-DS"; break;
  ...
  /*snprintf(tmp, sizeof(tmp)-1, "An unknown service is running on this port%s.\n",
      get_encaps_through(trp));
  if (norm != NULL)
    {
      strcat(tmp, "It is usually reserved for ");
      strcat(tmp, norm);
    }*/
  if (norm != NULL)
    snprintf(tmp, sizeof(tmp)-1, "Maybe the \"%s\" service running on this port%s.\n",
      norm, get_encaps_through(trp));
  else
    snprintf(tmp, sizeof(tmp)-1, "An unknown service is running on this port%s.\n",
      get_encaps_through(trp));
...
typedef struct _FIND_SERVICE_THREAD_INFO {
    struct arglist *desc;
    struct arglist *h;
    int *current_threads;
    int *plugin_threads;
    int test_ssl;
} FIND_SERVICE_THREAD_INFO;

DWORD WINAPI do_find_service(LPVOID lpParams)
{
    FIND_SERVICE_THREAD_INFO *thread_info = (FIND_SERVICE_THREAD_INFO *)lpParams;
    struct arglist *desc = thread_info->desc;
    struct arglist *h = emalloc(sizeof(struct arglist));
    int test_ssl = thread_info->test_ssl;
 ...
 bzero(unknown, sizeof(unknown));
     ...
     !strncmp(h->name, head, len_head))
  {
    ...
    if (type == ARG_STRING  && banner != NULL && banner[0] != '\0' /* strlen > 0*/)
      {
#ifdef _DEBUG
	fprintf(stderr, "Banner is known on port %d - will not open a new connection\n", port);
#endif
	cnx = -1;
	trp = NESSUS_ENCAPS_IP;
      ...
      {
#ifdef _DEBUG
	fprintf(stderr, "Banner is unknown on port %d - connecting...\n", port);
#endif
	if (banner != NULL)
	  efree (&banner);
	banner = NULL;
  ...
  if(cnx >= 0 || banner != NULL)
   {
    ...
    if (IS_ENCAPS_SSL(trp))
      {
	char	report[160] = {0};
        snprintf(report, sizeof(report)-1, "A %s server answered on this port\n", 
		get_encaps_name(trp));
	post_note(desc, port, report);
...
select_again:      
		...
		if (x < 0)
          {
           //if ( errno == EINTR ) goto select_again;
           if ( WSAGetLastError() == WSAEINTR ) goto select_again;
           perror("select");
          }
		else if (x == 0)
		  timeout = 1;
	    ...
	    if (FD_ISSET(realfd, &rfds))
	      {
			len = read_stream_connection_min(cnx, buffer, 1, sizeof(buffer)-1);
	      }
	    }
		t2 = time(NULL);
		delta_t = t2 - t1;
...
#if 1
	/* Try to work around broken web server (or "magic read" bug??) */
   ...
   if (len > 0)
       ...
       (banner[3] == '\0' || isspace(banner[3]) || banner[3] == '-'))
     {
       	...
       	/* Do *not* set Known/tcp/<port> to "three_digits": the service must
		* remain "unknown" */
       three_digits = 1;
      }

   if (get_sent)
     sprintf(kb, "FindService/tcp/%d/get_http", port);
   else
     sprintf(kb, "FindService/tcp/%d/spontaneous", port);
   plug_set_key(desc, kb, ARG_STRING, banner);
   ...
   if( strchr(origline, '\n') != NULL ){
	...
	mark_direct_connect_hub(desc, port, trp);
    ...
    else if(strstr(line, "220 ***************") || strstr(line, "220 eSafe@")) /* CISCO SMTP (?) - see bug #175 */
        mark_smtp_server(desc, port, origline, trp);
       ...
       mark_smtp_server(desc, port, origline, trp);
    else if( (strncmp(line + 1, "host '", 6) == 0 ) && strstr(line, "mysql") != NULL )
...
select_again2:
	   ...
	   if (x < 0)
             {
              //if ( errno == EINTR ) goto select_again2;
              if ( WSAGetLastError() == WSAEINTR ) goto select_again2;
              perror("select");
              }
	   else if (x > 0)
	     {
...
and in %d when we just wait. It is  probably not wrapped\n",
 ...
 arg_free_all(h);
 if (thread_info->current_threads)
     (*thread_info->current_threads)--;
 if (thread_info->plugin_threads)
     (*thread_info->plugin_threads)--;
 efree(&thread_info);
 return(0);

}

static int find_service_plugin_do_run(desc, h, test_ssl)
 struct arglist * desc;
 struct arglist * h;
 int test_ssl;
{
 FIND_SERVICE_THREAD_INFO *thread_info = NULL;
 int max_threads = plug_get_parameter(desc, "max_threads");
 int *current_threads = plug_get_parameter(desc, "curr_threads");
 int *plugin_threads = plug_get_parameter(desc, "plugin_threads");
 int orig_threads = *plugin_threads;
 DWORD thread_id = 0;

 if (!max_threads || !current_threads || !plugin_threads) return 0;

 while (h && h->next)
 {
  while ((*current_threads) >= max_threads) Sleep(10);
  (*current_threads)++;
  (*plugin_threads)++;
  thread_info = emalloc(sizeof(FIND_SERVICE_THREAD_INFO));
  thread_info->desc = desc;
  thread_info->h = h;
  thread_info->current_threads = current_threads;
  thread_info->plugin_threads = plugin_threads;
  thread_info->test_ssl = test_ssl;
  CreateThread(NULL, 0, do_find_service, thread_info, 0, &thread_id);
  ...
  while ((*plugin_threads) > orig_threads) Sleep(100);
 }
...
#ifdef NEW_KB_MGMT /* In Nessus 2.1.x and newer, the KB items are preceeded by their length */
 ...
 if((err = select(in + 1, &rd, NULL, NULL, &tv)) > 0)
 {
  ...
  if( n <= 1 )
  ...
  else
   {
    ...
    if ( err == 0 ) return 0;
    if ( WSAGetLastError() == WSAEINTR )errno = 0;
    else break;
    }
 }
 return 0; 
...
int find_service_plugin_run(desc)
 struct arglist * desc;
{
#ifdef NEW_KB_MGMT
 ...
 int	test_ssl = 0;
  ...
  find_service_plugin_do_run(desc, h, test_ssl); 
 ...
 for (i = 0; i < num_sons; i ++)
 {
  ...
  { 
   ...
   if(sons[i] == 0)
   {
    ...
    find_service_plugin_do_run(desc, sons_args[i], test_ssl);
    exit(0);
   }
   else {
	close(sons_pipe[i][0]);
	if ( sons[i] < 0 ) sons[i] = 0; // Fork failed
	}
  }
 }
