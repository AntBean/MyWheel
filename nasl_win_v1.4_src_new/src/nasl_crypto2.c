...
tree_cell * nasl_bn_cmp(lex_ctxt* lexic)
{
 ...
 if ( s1 == NULL || s2 == NULL )
  ...
  if (BN_bin2bn((const char*)s1, sz1, key1) == 0)
    goto fail;
  if (BN_bin2bn((const char*)s2, sz2, key2) == 0)
    goto fail;
...
tree_cell * nasl_dh_generate_key(lex_ctxt* lexic)
{
  ...
  if (BN_bin2bn((const char*)s1, sz1, dh->p) == 0)
     goto fail;
  if (BN_bin2bn((const char*)s2, sz2, dh->g) == 0)
     goto fail;
  if (BN_bin2bn((const char*)s3, sz3, dh->priv_key) == 0)
     goto fail;
...
tree_cell * nasl_dh_compute_key(lex_ctxt* lexic)
{
  ...
  if (BN_bin2bn((const char*)s2, sz2, dh->g) == 0)
     goto fail;
  if (BN_bin2bn((const char*)s3, sz3, dh_server_pub) == 0)
     goto fail;
  if (BN_bin2bn((const char*)s4, sz4, dh->pub_key) == 0)
     goto fail;
  if (BN_bin2bn((const char*)s5, sz5, dh->priv_key) == 0)
     goto fail;
...
tree_cell * nasl_rsa_public_decrypt(lex_ctxt* lexic)
{
  ...
  if (BN_bin2bn((const char*)s3, sz3, rsa->n) == 0)
    goto fail;
  if (BN_bin2bn((const char*)s2, sz2, rsa->e) == 0)
    goto fail;
  ...
  if ((len = RSA_public_decrypt(sz1, s1, decrypted, rsa,
	    RSA_PKCS1_PADDING)) < 0)
    goto fail;
...
tree_cell * nasl_rsa_sign(lex_ctxt* lexic)
{
  ...
  if (BN_bin2bn((const char*)s2, sz2, rsa->e) == 0)
    goto fail;
  if (BN_bin2bn((const char*)s4, sz4, rsa->d) == 0)
    goto fail;
  ...
  ok = RSA_sign(NID_sha1, s1, sz1, sig, &len, rsa);
  if (!ok || len > slen)
    goto fail;
...
tree_cell * nasl_bf_cbc(lex_ctxt* lexic, int enc)
{
  ...
  BF_set_key(&key, 16, enckey);

  out = emalloc(datalen);
  if (!out)
    goto fail;

  BF_cbc_encrypt(data, out, datalen, &key, iv, enc);
...
tree_cell * nasl_dsa_do_verify(lex_ctxt* lexic)
{
  ...
  if (BN_bin2bn((const char*)p, plen, dsa->p) == 0)
     goto fail;
  if (BN_bin2bn((const char*)g, glen, dsa->g) == 0)
     goto fail;
  if (BN_bin2bn((const char*)q, qlen, dsa->q) == 0)
     goto fail;
  if (BN_bin2bn((const char*)pub, publen, dsa->pub_key) == 0)
     goto fail;
  if (BN_bin2bn((const char*)r, rlen, sig->r) == 0)
     goto fail;
  if (BN_bin2bn((const char*)s, slen, sig->s) == 0)
     goto fail;

  if (DSA_do_verify(data, datalen, sig, dsa))
     retc->x.i_val = 1;
...
tree_cell * nasl_dsa_do_sign(lex_ctxt* lexic)
{
  ...
  if (BN_bin2bn((const char*)pub, publen, dsa->pub_key) == 0)
     goto fail;
  if (BN_bin2bn((const char*)priv, privlen, dsa->priv_key) == 0)
     goto fail;

  sig = DSA_do_sign(data, datalen, dsa);
  if (!sig)
    goto fail;  
...
char * map_file(char * filename, int * len) 
{
 ...
 map = emalloc(st.st_size+1);
 read_len = read(fd, map, st.st_size);
...
int generate_signed_script(char * filename)
{
 ...
 SHA1(msg, msg_len + sizeof(msg_len), md);
 if ( fp == NULL ) 
	{
	perror("open ");
	return -1;
...
int verify_script_signature(char * filename)
{
 ...
 SHA1(t, msg_len + sizeof(msg_len), md);
